...
#undef rs
    int n;
    stdvectorint in, out;
    stdvectorstdvectorstdpairint, int v;
};
...
int main() {
    ...
    auto &graph = tr.v;
    int m = tr.v.size() - 1;
    stdvectori64 dp(m + 1, LLONG_MAX);
    stdpriority_queuestdpairi64, int, stdvectorstdpairi64, int, stdgreater pq;
    pq.emplace(0LL, s);
    while(!pq.empty()) {
        auto [w, id] = pq.top();
        pq.pop();
    ...
    for(int i = 1; i = n; ++i) {
        stdcout  (dp[i] == LLONG_MAX  -1  dp[i])   n[i == n];
    }
    return 0;
}
